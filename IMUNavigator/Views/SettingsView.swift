...
class SensorFusionEngine: NSObject, ObservableObject, ARSessionDelegate, CLLocationManagerDelegate {
    ...
    private var pureImuVx: Double = 0.0
    private var pureImuVy: Double = 0.0
    private var hasNewMLVelocity: Bool = false
        ...
        engineQueue.async { [weak self] in guard let self = self else { return }
            ...
            if actStr == "Stationary" || isPhysicallyStationary {
                ...
                if data.timestamp - self.lastMLPredictTime >= 0.1 {
                    ...
                    RoNINModelWrapper.shared.predictVelocity(currentTime: data.timestamp) { [weak self] vel, msg in
                        ...
                        self.engineQueue.async {
                            ...
                            if let v = vel { 
                                self.latestMLVelocity = v
                                self.hasNewMLVelocity = true
                            }
                        }
                ...
                if let mlVel = self.latestMLVelocity {
                    ...
                    if self.hasNewMLVelocity {
                        let currentX = vx_global - self.pureImuVx
                        let currentY = vy_global - self.pureImuVy
                        DispatchQueue.main.async {
                            self.currentResX = currentX
                            self.currentResY = currentY
                        }
                        self.pureImuVx = vx_global
                        self.pureImuVy = vy_global
                        self.hasNewMLVelocity = false
                    } else {
                        self.pureImuVx += correctedAcc.x * dt
                        self.pureImuVy += correctedAcc.y * dt
                    }
                } else {
                    ...
                    self.pureImuVx = damped_v_xy.x
                    self.pureImuVy = damped_v_xy.y
                    DispatchQueue.main.async {
                        self.currentResX = 0.0
                        self.currentResY = 0.0
                    }
                }
            }
