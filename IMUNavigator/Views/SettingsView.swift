        ...
        NavigationView {
            Form {
                Section(header: Text(l("Core Routing Engine", "核心路由引擎"))) {
                    ...
                    HStack { Text(l("Bias X", "偏置 X")); Spacer(); TextField("X", text: $biasXStr).keyboardType(.decimalPad).multilineTextAlignment(.trailing).focused($isInputActive).frame(width: 80).onChange(of: biasXStr) { _, newValue in if let d = Double(newValue) { AppSettings.shared.manualBiasX = d } } }
                    HStack { Text(l("Bias Y", "偏置 Y")); Spacer(); TextField("Y", text: $biasYStr).keyboardType(.decimalPad).multilineTextAlignment(.trailing).focused($isInputActive).frame(width: 80).onChange(of: biasYStr) { _, newValue in if let d = Double(newValue) { AppSettings.shared.manualBiasY = d } } }
                    HStack { Text(l("Bias Z", "偏置 Z")); Spacer(); TextField("Z", text: $biasZStr).keyboardType(.decimalPad).multilineTextAlignment(.trailing).focused($isInputActive).frame(width: 80).onChange(of: biasZStr) { _, newValue in if let d = Double(newValue) { AppSettings.shared.manualBiasZ = d } } }
                ...
                Section(header: Text(l("Recording Mode", "记录模式"))) {
                    ...
                    if recordingModeIndex == 0 {
                        HStack { Label(l("Time Interval (s)", "时间间隔 (秒)"), systemImage: "clock"); Spacer(); TextField(l("0 = No limit", "0 = 无限制"), text: $timeStr).keyboardType(.decimalPad).multilineTextAlignment(.trailing).focused($isInputActive).onChange(of: timeStr) { _, newValue in if let d = Double(newValue) { AppSettings.shared.recordIntervalTime = d } } }
                    } else {
                        HStack { Label(l("Space Interval (m)", "距离间隔 (米)"), systemImage: "ruler"); Spacer(); TextField(l("0 = No limit", "0 = 无限制"), text: $spaceStr).keyboardType(.decimalPad).multilineTextAlignment(.trailing).focused($isInputActive).onChange(of: spaceStr) { _, newValue in if let d = Double(newValue) { AppSettings.shared.recordIntervalSpace = d } } }
                    }
                }
                ...
                Section(header: Text(l("Algorithm Control", "算法控制"))) {
                    ...
                    HStack { Label(l("Accel Threshold", "加速度阈值"), systemImage: "speedometer"); Spacer(); TextField("Accel", text: $zuptAccStr).keyboardType(.decimalPad).multilineTextAlignment(.trailing).focused($isInputActive).onChange(of: zuptAccStr) { _, newValue in if let d = Double(newValue) { AppSettings.shared.zuptThreshold = d } } }
                    HStack { Label(l("Gyro Threshold", "陀螺仪阈值"), systemImage: "gyroscope"); Spacer(); TextField("Gyro", text: $zuptGyroStr).keyboardType(.decimalPad).multilineTextAlignment(.trailing).focused($isInputActive).onChange(of: zuptGyroStr) { _, newValue in if let d = Double(newValue) { AppSettings.shared.zuptGyroThreshold = d } } }
                }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 15) {
            ...
            if chartModeIndex == 0 {
                ...
                let chart1Min: Double = {
                    var vals: [Double] = []
                    if showAccX { vals.append(displayPoints.compactMap { $0.acceleration?.x }.min() ?? 0) }
                    if showAccY { vals.append(displayPoints.compactMap { $0.acceleration?.y }.min() ?? 0) }
                    if showSpeed { vals.append(displayPoints.map { $0.speed }.min() ?? 0) }
                    return vals.min() ?? 0
                }()
                
                let chart1Max: Double = {
                    var vals: [Double] = []
                    if showAccX { vals.append(displayPoints.compactMap { $0.acceleration?.x }.max() ?? 0) }
                    if showAccY { vals.append(displayPoints.compactMap { $0.acceleration?.y }.max() ?? 0) }
                    if showSpeed { vals.append(displayPoints.map { $0.speed }.max() ?? 0) }
                    return vals.max() ?? 0
                }()
                ...
                let chart2Min: Double = {
                    var vals: [Double] = []
                    if showResX { vals.append(displayPoints.compactMap { $0.resX }.min() ?? 0) }
                    if showResY { vals.append(displayPoints.compactMap { $0.resY }.min() ?? 0) }
                    return vals.min() ?? 0
                }()
                
                let chart2Max: Double = {
                    var vals: [Double] = []
                    if showResX { vals.append(displayPoints.compactMap { $0.resX }.max() ?? 0) }
                    if showResY { vals.append(displayPoints.compactMap { $0.resY }.max() ?? 0) }
                    return vals.max() ?? 0
                }()
